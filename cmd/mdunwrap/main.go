// mdunwrap unwraps Markdown paragraphs to one sentence per line.
//
// Usage:
//
//	mdunwrap [file...]
...
// unwrapParagraph joins lines and splits into sentences.
...
// A sentence ends with . ! or ? followed by a space and a capital letter.
...
// unwrapBlockquote unwraps blockquote lines to one sentence per line.
	...
	for _, line := range lines {
		...
		if strings.HasPrefix(content, "[!") && strings.Contains(content, "]") {
			...
			if len(contentLines) > 0 {
				unwrapped := unwrapToSentences(contentLines)
...
// unwrapToSentences joins lines and splits into sentences.
func unwrapToSentences(lines []string) []string {
	text := strings.Join(lines, " ")
	text = strings.Join(strings.Fields(text), " ")
